...
class producto {
    ...
    public function listadoProducto($param=null) {
        $this->conexion(1);
        //print_r($param);
        if(!empty($param)){
        ...
        if(empty($where)){
            $query = "SELECT * FROM `".$this->tabla."` ORDER BY `codigoID` ASC $limit;";
        }else{
            $query = "SELECT * FROM `".$this->tabla."` ".$where." ORDER BY `codigoID` ASC $limit;";
        }
        try {
            if ($result = $this->mysqli->query($query)) {
                if ($result->num_rows === 0){
                ...
                while ($row = $result->fetch_array()){
                    foreach ($campos as $key => $value) {
                        $datos[$row['codigoID']][$value]=$row[$value];
                    }
                }
                $result->close();
            }else{
