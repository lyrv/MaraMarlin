...
class codigospostales {
    ...
    public $mensaje;
    public $campos;
    public $tabla = "codigospostales";
    public $vista = "vw_codigospostales";
    private $rows;
    ...
    public function listadoCodigosPostales($param=null) {
        $this->conexion(1);
        //print_r($param);
        if(!empty($param)){
            if(!empty($param['CodigoPostal'])){
                $where=" WHERE ";
                $where.= "CodigoPostal = '".$param['CodigoPostal']."'";
            }
        ...
        if(empty($where)){
            $query = "SELECT * FROM `".$this->tabla."` ORDER BY `CodigoPostal` ASC $limit;";
        }else{
            $query = "SELECT * FROM `".$this->tabla."` ".$where." ORDER BY `CodigoPostal` ASC $limit;";
        }
        try {
            if ($result = $this->mysqli->query($query)) {
                if ($result->num_rows === 0){
                ...
                while ($row = $result->fetch_array()){
                    foreach ($campos as $key => $value) {
                        $datos[$row['CodigoPostal']][$value]=$row[$value];
                    }
                }
                $result->close();
            }else{
